  ...
  const followupDays: number[] = (customer.store?.followupDays as any) || [4, 12];
      ...
      for (const followupDay of followupDays) {
        ...
        if (daysSinceEvent >= followupDay && daysSinceEvent < followupDay + 1) {
          ...
          let message: string;
          if (isFirstFollowup) {
            message = `Had a chance to tryout your ${customer.sampleChoice}, yet? If you loved it, ${customer.store?.storeName} is still offering ${customer.store?.promoOffer} on your first purchase if you're interested!\n\nClick link to redeem: ${baseUrl}/p/${customer.promoSlug}\n\nReply STOP to opt out.`;
